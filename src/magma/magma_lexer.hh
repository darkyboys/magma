...
namespace magma {
    darr lexer (func code_line){
        ...
        bool is_array_opened = false, is_array_already_loaded = false;
        ...
        for (_func_ i = 0;i < code_line.length();i++){
            ...
            // Array
            else if (code_line[i] == ',' && is_array_opened && lexer_mstring.is_String(i) == 0){
                value += SEPERATOR;
            }
            else if (code_line[i] == ']' && is_array_opened && lexer_mstring.is_String(i) == 0){
                is_array_opened = false;
                is_array_already_loaded = true;
                value += SEPERATOR;
                // std::cout<<"Collision Detected : The Function is now closed\n";
            }
            else if (code_line[i] == '[' && is_array_opened == false && lexer_mstring.is_String(i) == 0){
